...
CMyString::operator+ (const char *psz)
{
    return this->StringCat(psz);
}

CMyString
CMyString::operator+ (const CMyString &obj)
{
    return this->StringCat(obj);
}

CMyString& 
CMyString::operator+= (const char *psz)
{
    return StringCat(psz);
}

CMyString& 
CMyString::operator+= (const CMyString &obj)
{
    return StringCat(obj);
}

char &
CMyString::operator[] (int nIndex)
{
    if (nIndex >= 0
        && nIndex <= m_nlen)
    {
...
CMyString::operator> (const CMyString &obj)
{
    return this->operator > (obj.m_psz);
}
...
CMyString::SetString(const char *psz)
{
    ...
    */
    ...
    //Modified by wbxu 20140507, no need to new m_prefcount, we just use the former one.
    if (NULL == m_prefcount)
    {
        m_prefcount = new int;
        if (NULL == m_prefcount)
